...
void vkglTF::Texture::fromglTfImage(AAssetManager* assetManager, tinygltf::Image &gltfimage, std::string path, vks::VulkanDevice *device, VkQueue copyQueue)
{
	this->device = device;
...
void vkglTF::Model::loadImages(AAssetManager* assetManager, tinygltf::Model &gltfModel, vks::VulkanDevice *device, VkQueue transferQueue)
{
	for (tinygltf::Image &image : gltfModel.images) {
		vkglTF::Texture texture;
		texture.fromglTfImage(assetManager, image, path, device, transferQueue);
...
void vkglTF::Model::loadFromFile(AAssetManager* assetManager, std::string filename, vks::VulkanDevice *device, VkQueue transferQueue, uint32_t fileLoadingFlags, float scale)
{
	...
	if (fileLoaded) {
		if (!(fileLoadingFlags & FileLoadingFlags::DontLoadImages)) {
			loadImages(assetManager, gltfModel, device, transferQueue);
		}
