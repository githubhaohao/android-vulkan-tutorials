 ...
 *                              that required for the specified outputWidth, outputHeight
             ...
             GLuint numImages, KTX_image_info images[])
{
    ...
    {
        /* texture must have outputWidth */
        /* texture must have outputHeight if it has depth */
        return KTX_INVALID_VALUE;
    }
    ...
    if (numMipmapLevels > 1) {
        ...
        {
            /* Can't have more mip levels than 1 + log2(max(outputWidth, outputHeight, depth)) */
            return KTX_INVALID_VALUE;
        }
    }
    ...
    for (level = 0, i = 0; level < numMipmapLevels; ++level)
    {
        ...
        /* Calculate face sizes for this LoD based on glType, glFormat, outputWidth & outputHeight */
