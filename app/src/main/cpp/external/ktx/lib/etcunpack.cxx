			  ...
			  GLint R16Formats, GLboolean supportsSRGB)
{
    ...
    /* active_{outputWidth,outputHeight} show how many pixels contain active data,
	 * (the rest are just for making sure we have a 2*a x 4*b size).
	 */

	/* Compute the full outputWidth & outputHeight. */
	...
	*dstImage = (GLubyte*)malloc(dstChannels*dstChannelBytes*width*height);
	...
	// NOTE: none of the decompress functions actually use the <outputHeight> parameter
