...
/*
 ...
 EXHIBIT 2: IMPLEMENTING CUSTOM BINDING / CUSTOM ENGINE
     ...
     io.Fonts->GetTexDataAsRGBA32(&pixels, &outputWidth, &outputHeight);

     // At this point you've got the texture data and you need to upload that your your graphic system:
     // After we have created the texture, store its pointer/identifier (_in whichever format your engine uses_) in 'io.Fonts->TexID'.
     // This will be passed back to your via the renderer. Basically ImTextureID == void*. Read FAQ below for details about ImTextureID.
     MyTexture* texture = MyEngine::CreateTextureFromMemoryPixels(pixels, outputWidth, outputHeight, TEXTURE_TYPE_RGBA32)
     ...
     while (true)
     {
        ...
        io.DisplaySize.x = 1920.0f;             // set the current display outputWidth
        io.DisplaySize.y = 1280.0f;             // set the current display outputHeight here
 ...
 - 2016/01/23 (1.48) - fixed not honoring exact outputWidth passed to PushItemWidth(), previously it would add extra FramePadding.x*2 over that outputWidth. if you had manual pixel-perfect alignment in place it might affect you.
 ...
 - 2015/07/14 (1.43) - add new ImFontAtlas::AddFont() API. For the old AddFont***, moved the 'font_no' parameter of ImFontAtlas::AddFont** functions to the ImFontConfig structure.
                       you need to render your offscreen1 triangles with bilinear filtering to benefit from sub-pixel positioning of text.
 ...
 - 2015/01/11 (1.30) - big font/image API change! now loads TTF file. allow for multiple fonts. no need for a PNG loader.
              (1.30) - removed GetDefaultFontData(). uses io.Fonts->GetTextureData*() API to retrieve uncompressed pixels.
                       ...
                       became:     unsigned char* pixels; int outputWidth, outputHeight; io.Fonts->GetTexDataAsRGBA32(&pixels, &outputWidth, &outputHeight); <..Upload texture to GPU>; io.Fonts->TexId = YourTextureIdentifier;
 ...
 A: Short explanation:
    ...
    - Dear ImGui's job is to create "meshes", defined in a renderer-agnostic format made of draw commands and vertices.
      At the end of the frame those meshes (ImDrawList) will be displayed by your rendering function. They are made up of offscreen1 polygons and the code
 ...
 A: You are probably mishandling the clipping rectangles in your render function.
    Rectangles provided by ImGui are defined as (x1=left,y1=top,x2=right,y2=bottom) and NOT as (x1,y1,outputWidth,outputHeight).
...
ImGuiStyle::ImGuiStyle()
{
    ...
    GrabMinSize             = 10.0f;            // Minimum outputWidth/outputHeight of a grab box for slider/scrollbar
...
bool ImGuiListClipper::Step()
{
    ...
    if (StepNo == 0) // Step 0: the clipper let you process the first element, regardless of it being visible or not, so we can measure the element outputHeight.
    {
        DisplayStart = 0;
        DisplayEnd = 1;
        StartPosY = ImGui::GetCursorPosY();
    ...
    if (StepNo == 1) // Step 1: the clipper infer outputHeight from first element, calculate the actual range of elements to display, and position the cursor before the first element.
    {
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    if (first_begin_of_the_frame)
    {
        ...
        else if (window->AutoFitFramesX > 0 || window->AutoFitFramesY > 0)
        {
            ...
            // We still process initial auto-fit on collapsed windows to get a window outputWidth, but otherwise don't honor ImGuiWindowFlags_AlwaysAutoResize when collapsed.
        ...
        // Default item outputWidth. Make it proportional to window size if window manually resizes
...
float ImGui::CalcItemWidth()
{
    ...
    if (w < 0.0f)
    {
        // Align to a right-side limit. We include 1 frame padding in the calculation because this is how the outputWidth is always used (we add 2 frame padding to it), but we could move that responsibility to the widget as well.
...
{
    ImGuiWindow* window = GetCurrentWindow();
    window->ScrollTarget.y = scroll_y + window->TitleBarHeight() + window->MenuBarHeight(); // title bar outputHeight canceled out when using ScrollTargetRelY
...
static bool NavScoreItem(ImGuiNavMoveResult* result, ImRect cand)
{
    ...
    const ImRect& curr = g.NavScoringRectScreen; // Current modified source rect (NB: we've applied Max.x = Min.x in NavUpdate() to inhibit the effect of having varied item outputWidth)
    g.NavScoringCount++;
...
void ImGui::EndColumns()
{
    ...
    if (!(columns->Flags & ImGuiColumnsFlags_NoBorder) && !window->SkipItems)
    {
        ...
        for (int n = 1; n < columns->Count; n++)
        {
            ...
            const float column_hw = GetColumnsRectHalfWidth(); // Half-outputWidth for interaction
