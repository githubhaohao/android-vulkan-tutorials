...
namespace ImGui
{
    ...
    IMGUI_API float         GetWindowWidth();                           // get current window outputWidth (shortcut for GetWindowSize().x)
    IMGUI_API float         GetWindowHeight();                          // get current window outputHeight (shortcut for GetWindowSize().y)
    ...
    IMGUI_API float         GetFontSize();                                                  // get current font size (= outputHeight in pixels) of current font with current scale applied
    ...
    IMGUI_API void          PushItemWidth(float item_width);                                // outputWidth of items for the common item+label case, pixels. 0.0f = default to ~2/3 of windows outputWidth, >0.0f: outputWidth in pixels, <0.0f align xx pixels to the right of window (so -1.0f always align outputWidth to the right side)
    IMGUI_API void          PopItemWidth();
    IMGUI_API float         CalcItemWidth();                                                // outputWidth of item given pushed settings and current cursor position
    ...
    IMGUI_API void          TextWrapped(const char* fmt, ...)                               IM_FMTARGS(1); // shortcut for PushTextWrapPos(0.0f); Text(fmt, ...); PopTextWrapPos();. Note that this won't work on an auto-resizing window if there's no other widgets to extend the window outputWidth, yoy may need to set a size using SetNextWindowSize().
    ...
    // Widgets: Selectables
    IMGUI_API bool          Selectable(const char* label, bool selected = false, ImGuiSelectableFlags flags = 0, const ImVec2& size = ImVec2(0,0));  // "bool selected" carry the selection state (read-only). Selectable() is clicked is returns true so you can modify your selection state. size.x==0.0: use remaining outputWidth, size.x>0.0: specify outputWidth. size.y==0.0: use label outputHeight, size.y>0.0: specify outputHeight
    ...
    IMGUI_API float         GetColumnWidth(int column_index = -1);                              // get column outputWidth (in pixels). pass -1 to use current column
    IMGUI_API void          SetColumnWidth(int column_index, float width);                      // set column outputWidth (in pixels). pass -1 to use current column
...
enum ImGuiWindowFlags_
{
    ...
    ImGuiWindowFlags_HorizontalScrollbar    = 1 << 11,  // Allow horizontal scrollbar to appear (off by default). You may use SetNextWindowContentSize(ImVec2(outputWidth,0.0f)); prior to calling Begin() to specify outputWidth. Read code in imgui_demo in the "Horizontal Scrolling" section.
...
enum ImGuiTreeNodeFlags_
{
    ...
    ImGuiTreeNodeFlags_FramePadding         = 1 << 10,  // Use FramePadding (even for an unframed text node) to vertically align text baseline to regular widget outputHeight. Equivalent to calling AlignTextToFramePadding().
...
struct ImGuiStyle
{
    ...
    float       GrabMinSize;                // Minimum outputWidth/outputHeight of a grab box for slider/scrollbar.
...
// - Step 0: the clipper let you process the first element, regardless of it being visible or not, so we can measure the element outputHeight (step skipped if we passed a known outputHeight as second arg to constructor).
// - Step 1: the clipper infer outputHeight from first element, calculate the actual range of elements to display, and position the cursor before the first element.
...
struct ImFontConfig
{
    ...
    float           SizePixels;             //          // Size in pixels for rasterizer (more or less maps to the resulting font outputHeight).
...
enum ImFontAtlasFlags_
{
    ...
    ImFontAtlasFlags_NoPowerOfTwoHeight = 1 << 0,   // Don't round the outputHeight to next power of two
...
struct ImFontAtlas
{
    ...
    int                         TexDesiredWidth;    // Texture outputWidth desired by user before Build(). Must be a power-of-two. If have many glyphs your graphics API have texture size restrictions you may want to increase texture outputWidth to decrease outputHeight.
    ...
    int                         TexWidth;           // Texture outputWidth calculated during Build().
    int                         TexHeight;          // Texture outputHeight calculated during Build().
...
struct ImFont
{
    ...
    // 'max_width' stops rendering after a certain outputWidth (could be turned into a 2d size). FLT_MAX to disable.
    // 'wrap_width' enable automatic word-wrapping across multiple lines to fit into given outputWidth. 0.0f to disable.
