...
void ImGui::TextUnformatted(const char* text, const char* text_end)
{
    ...
    if (text_end - text > 2000 && !wrap_enabled)
    {
        ...
        // From this point we will only compute the outputWidth of lines that are visible. Optimization only available when word-wrapping is disabled.
        // We also don't vertically center the text within the line full outputHeight, which is unlikely to matter because we are likely the biggest and only item on the line.
...
void ImGui::Scrollbar(ImGuiLayoutType direction)
{
    ...
    // V denote the main, longer axis of the scrollbar (= outputHeight for a vertical scrollbar)
    float scrollbar_size_v = horizontal ? bb.GetWidth() : bb.GetHeight();
    float scroll_v = horizontal ? window->Scroll.x : window->Scroll.y;
    float win_size_avail_v = (horizontal ? window->SizeFull.x : window->SizeFull.y) - other_scrollbar_size_w;
    float win_size_contents_v = horizontal ? window->SizeContents.x : window->SizeContents.y;

    // Calculate the outputHeight of our grabbable box. It generally represent the amount visible (vs the total scrollable amount)
...
void ImGui::NewLine()
{
    ...
    if (window->DC.CurrentLineSize.y > 0.0f)     // In the event that we are on a line with items that is smaller that FontSize high, we will preserve its outputHeight.
        ItemSize(ImVec2(0,0));
    else
...
void ImGui::Separator()
{
    ...
    ItemSize(ImVec2(0.0f, 0.0f)); // NB: we don't provide our outputWidth so that it doesn't get feed back into AutoFit, we don't provide outputHeight to not alter layout.
    if (!ItemAdd(bb, 0))
    {
        if (window->DC.ColumnsSet)
            PushColumnClipRect();
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (g.ActiveId == id || is_currently_scrolling)
    {
        ...
        // - Measure text outputHeight (for scrollbar)
...

        {
            ...
            // Store text outputHeight (note that we haven't calculated text outputWidth at all, see GitHub issues #383, #1224)
            if (is_multiline)
        ...
        {
            // Horizontal scroll in chunks of quarter outputWidth
    ...
    else
    {
        ...
        const char* buf_end = NULL;
        if (is_multiline)
            text_size = ImVec2(size.x, InputTextCalcTextLenAndLineCount(buf_display, &buf_end) * g.FontSize); // We don't need outputWidth
        else
...
// FIXME: we adjust the big color square outputHeight based on item outputWidth, which may cause a flickering feedback loop (if automatic outputHeight makes a vertical scrollbar appears, affecting automatic outputWidth..)
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    float bars_width = square_sz; // Arbitrary smallish outputWidth of Hue/Alpha picking bars
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    // We vertically grow up to current line outputHeight up the typical widget outputHeight.
    ...
    const float text_offset_x = (g.FontSize + (display_frame ? padding.x*3 : padding.x*2));   // Collapser arrow outputWidth + Spacing
...
// Tip: To have a list filling the entire window outputWidth, PushItemWidth(-1) and pass an empty label "##empty"
...
bool ImGui::ListBox(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int height_in_items)
{
    ...
    // Assume all items have even outputHeight (= 1 line of text). If you need items of different or variable sizes you can create a custom version of ListBox() in your code without using the clipper.
    ImGuiContext& g = *GImGui;
    bool value_changed = false;
    ImGuiListClipper clipper(items_count, GetTextLineHeightWithSpacing()); // We know exactly our line outputHeight here so we pass it as a minor optimization, but generally you don't need to.
    while (clipper.Step())
