...
enum ImGuiColumnsFlags_
{
    ...
    ImGuiColumnsFlags_NoPreserveWidths      = 1 << 2,   // Disable column outputWidth preservation when adjusting columns
    ImGuiColumnsFlags_NoForceWithinWindow   = 1 << 3,   // Disable forcing columns to fit within window
    ImGuiColumnsFlags_GrowParentContentsSize= 1 << 4    // (WIP) Restore pre-1.51 behavior of extending the parent window contents size but _without affecting the columns outputWidth at all_. Will eventually remove.
};
...
struct ImGuiContext
{
    ...
    float                   FontSize;                           // (Shortcut) == FontBaseSize * g.CurrentWindow->FontWindowScale == window->FontSize(). Text outputHeight for current window.
    float                   FontBaseSize;                       // (Shortcut) == IO.FontGlobalScale * Font->Scale * Font->FontSize. Base text outputHeight.
...
struct IMGUI_API ImGuiWindowTempData
{
    ...
    float                   ItemWidth;              // == ItemWidthStack.back(). 0.0: default, >0.0: outputWidth in pixels, <0.0: align xx pixels to the right of window
...
namespace ImGui
{
    ...
    IMGUI_API void          VerticalSeparator();        // Vertical separator, for menu bars (use current line outputHeight). Not exposed because it is misleading and it doesn't have an effect on regular layout.
