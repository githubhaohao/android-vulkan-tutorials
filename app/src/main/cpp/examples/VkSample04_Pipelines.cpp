...
VkSample04_Pipelines::VkSample04_Pipelines() {

    title = "VkSample04_Pipelines";
    pTexture2D = nullptr;
}
...
void VkSample04_Pipelines::updateTransformMatrix(float rotateX, float rotateY, float scaleX, float scaleY)
{
...
void VkSample04_Pipelines::createVertexBuffer() {
...
void VkSample04_Pipelines::createDescriptors() {
    // Pool
...
void VkSample04_Pipelines::createPipelines() {
    // Layout
    ...
    //创建第二个 pipeline, 仅仅修改片段着色器的代码
    //Create a second pipeline and only modify the fragment shader code
    shaderStages[0] = compileShader(device, getShadersPath() + "pipelines/texture.vert", VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] = compileShader(device, getShadersPath() + "pipelines/hsv.frag", VK_SHADER_STAGE_FRAGMENT_BIT);
    VK_CHECK_RESULT(vkCreateGraphicsPipelines(device, pipelineCache, 1, &pipelineCreateInfo, nullptr, &pipeline2));
}

void VkSample04_Pipelines::createUniformBuffers() {
...
void VkSample04_Pipelines::buildCommandBuffers()
{
...
void VkSample04_Pipelines::loadTexture()
{
    if(pTexture2D) {
        pTexture2D->destroy();
        delete pTexture2D;
...
void VkSample04_Pipelines::prepare() {
...
void VkSample04_Pipelines::render() {
    if (!prepared)
        return;
