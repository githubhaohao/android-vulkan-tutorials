...
VkSample08_RenderYUYV::VkSample08_RenderYUYV() {

    title = "VkSample08_RenderYUYV";
...
void VkSample08_RenderYUYV::updateTransformMatrix(float rotateX, float rotateY, float scaleX, float scaleY)
{
    LOGCATE("VkSample08_RenderYUYV::updateTransformMatrix()");
...
void VkSample08_RenderYUYV::loadImage(NativeImage *pImage)
{
    LOGCATE("VkSample08_RenderYUYV::loadImage() pImage = %p", pImage->ppPlane[0]);
    assert(pImage->format == IMAGE_FORMAT_YUYV);
    if (pImage)
    {
...
void VkSample08_RenderYUYV::createVertexBuffer() {
...
void VkSample08_RenderYUYV::createDescriptors() {
    // Pool
...
void VkSample08_RenderYUYV::createPipelines() {
    // Layout
    ...
    // Shaders
    shaderStages[0] = compileShader(device, getShadersPath() + "yuv/yuyv.vert", VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] = compileShader(device,getShadersPath() + "yuv/yuyv.frag", VK_SHADER_STAGE_FRAGMENT_BIT);
...
void VkSample08_RenderYUYV::createUniformBuffers() {
...
void VkSample08_RenderYUYV::buildCommandBuffers()
{
...
void VkSample08_RenderYUYV::loadTexture()
{
    ...
    VkFormat kTextureFormat = VK_FORMAT_R8G8_UNORM;
    int bufferSize = renderImage.width * renderImage.height * 2;
    ...
    kTextureFormat = VK_FORMAT_R8G8B8A8_UNORM;
    pUvTexture2D->fromBuffer(renderImage.ppPlane[0], bufferSize, kTextureFormat, renderImage.width >> 1, renderImage.height, vulkanDevice, queue);
...
void VkSample08_RenderYUYV::prepare() {
...
void VkSample08_RenderYUYV::render() {
    if (!prepared)
        return;
