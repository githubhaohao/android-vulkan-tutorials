...
VkSample10_SpecializationInfo::VkSample10_SpecializationInfo() {

    title = "VkSample10_SpecializationInfo";
...
void VkSample10_SpecializationInfo::updateTransformMatrix(float rotateX, float rotateY, float scaleX, float scaleY)
{
...
void VkSample10_SpecializationInfo::createVertexBuffer() {
...
void VkSample10_SpecializationInfo::createDescriptors() {
    // Pool
...
void VkSample10_SpecializationInfo::createPipelines() {
    // Layout
    ...
    // Map entry for the split parameter
    specializationMapEntries[1].constantID = 1;
    specializationMapEntries[1].size = sizeof(specializationData.split);
    specializationMapEntries[1].offset = offsetof(SpecializationData, split);
    ...
    specializationData.split = 2.0f;

    // Shaders
    shaderStages[0] = compileShader(device, getShadersPath() + "specializationinfo/texture.vert", VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] = compileShader(device, getShadersPath() + "specializationinfo/texture.frag", VK_SHADER_STAGE_FRAGMENT_BIT);
...
void VkSample10_SpecializationInfo::buildCommandBuffers()
{
    ...
    for (int32_t i = 0; i < drawCmdBuffers.size(); ++i)
    {
        ...
        viewport.height = outputHeight / 2;
...
void VkSample10_SpecializationInfo::loadTexture()
{
    if(pTexture2D) {
        pTexture2D->destroy();
        delete pTexture2D;
    ...
    float viewportRatio = (float) outputWidth / (float) outputHeight * 2.0;
...
void VkSample10_SpecializationInfo::prepare() {
...
void VkSample10_SpecializationInfo::render() {
    if (!prepared)
        return;
