...
VkSample07_RenderI420::VkSample07_RenderI420() {

    title = "VkSample07_RenderI420";
...
VkSample07_RenderI420::~VkSample07_RenderI420() {
    ...
    for (int i = 0; i < 3; i++) {
        if(pYuvTexture2Ds[i]) {
            pYuvTexture2Ds[i]->destroy();
            delete pYuvTexture2Ds[i];
            pYuvTexture2Ds[i] = nullptr;
        }
    }
...
void VkSample07_RenderI420::updateTransformMatrix(float rotateX, float rotateY, float scaleX, float scaleY)
{
    LOGCATE("VkSample07_RenderI420::updateTransformMatrix()");
...
void VkSample07_RenderI420::loadImage(NativeImage *pImage)
{
    LOGCATE("VkSample07_RenderI420::loadImage() pImage = %p", pImage->ppPlane[0]);
    assert(pImage->format == IMAGE_FORMAT_I420);
    if (pImage)
    {
...
void VkSample07_RenderI420::createVertexBuffer() {
...
void VkSample07_RenderI420::createDescriptors() {
    // Pool
    ...
    std::vector<VkDescriptorSetLayoutBinding> setLayoutBindings = {
            ...
            vks::initializers::descriptorSetLayoutBinding(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, VK_SHADER_STAGE_FRAGMENT_BIT, 1, 3),
    };
    ...
    for (int i = 0; i < 3; i++) {
        imageInfo[i] = pYuvTexture2Ds[i]->descriptor;
    }
    ...
    std::vector<VkWriteDescriptorSet> writeDescriptorSets = {
            ...
            vks::initializers::writeDescriptorSet(descriptorSet,
                                                  ...
                                                  imageInfo, 3),								// Pointer to the descriptor image for our texture
    };
    vkUpdateDescriptorSets(device, static_cast<uint32_t>(writeDescriptorSets.size()), writeDescriptorSets.data(), 0, nullptr);
}

void VkSample07_RenderI420::createPipelines() {
    // Layout
    ...
    // Shaders
    shaderStages[0] = compileShader(device, getShadersPath() + "yuv/i420.vert", VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] = compileShader(device,getShadersPath() + "yuv/i420.frag", VK_SHADER_STAGE_FRAGMENT_BIT);
...
void VkSample07_RenderI420::createUniformBuffers() {
    ...
    bufferInfo.size = sizeof(MVPMatrix);
    ...
    VK_CHECK_RESULT(vkMapMemory(device, uniformBuffer.memory, 0, sizeof(MVPMatrix), 0,
                                (void **) &uniformBuffer.mapped));
}

void VkSample07_RenderI420::buildCommandBuffers()
{
...
void VkSample07_RenderI420::loadTexture()
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        pYuvTexture2Ds[i] = new vks::Texture2D();
        if(i == 0) { //y plane
            pYuvTexture2Ds[i]->fromBuffer(renderImage.ppPlane[i], bufferSize, kTextureFormat, renderImage.width, renderImage.height, vulkanDevice, queue);
        } else { //uv plane
            pYuvTexture2Ds[i]->fromBuffer(renderImage.ppPlane[i], bufferSize / 4, kTextureFormat, renderImage.width >> 1, renderImage.height >> 1, vulkanDevice, queue);
        }
    }

    float viewportRatio = (float) outputWidth / (float) outputHeight;
    float imageRatio = (float) pYuvTexture2Ds[0]->width / (float) pYuvTexture2Ds[0]->height;
...
void VkSample07_RenderI420::prepare() {
...
void VkSample07_RenderI420::render() {
    if (!prepared)
        return;
    VulkanExampleBase::prepareFrame();
    memcpy(uniformBuffer.mapped, &mvpMatrix, sizeof(MVPMatrix));
